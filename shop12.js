...
var shoptwelve_poll1=0;
   if ($("#shoptwelvecontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shoptwelve_poll1 = 0;
      //////console.log(shoptwelve_poll1);
   } else {
      shoptwelve_poll1 = 1;
      //////console.log(shoptwelve_poll1);
   }
...
var shoptwelve_machine_array = [];
var shoptwelve_hr_shop = "";
var shoptwelve_hr_shift = "";
var shoptwelve_hr_date = "";



   $(document).on('click', '.shop12_machine_name', function () {

      var shop=shoptwelve_name[0];
      var machine=$(this).text();
      var date=shoptwelve_hr_date;
      var shift_name=shoptwelve_hr_shift[0];
     
      var split_date=shop12_prev_date.split("-");

      $("html").toggleClass("sidebar-left-opened");
      window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shop12_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shoptwelve_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shoptwelve_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop12_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop12_machine_name font_16');
               td.setAttribute('id', 'shop_twelve' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shoptwelve_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shoptwelve_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_twelve' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shoptwelve_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shoptwelve_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shoptwelve_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shoptwelve_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shoptwelve_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shoptwelve_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shoptwelve_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwelve&type=json&ip=" + shoptwelve_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shoptwelve_name = da.shop;
            shoptwelve_machine_array = da.machine;
            shop12_prev_shift=da.prev_shift[0];
            shop12_prev_date=da.prev_date[0];
            //console.log(shop12_prev_date);
            $("#shoptwelve_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[12].text = da.shop;
            $('#shop_twelve0' + ic + '0').html(da.machine[i]);
            ...
            shoptwelve_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shoptwelve_hr_shop = da.shop;
            shoptwelve_hr_shift = da.shift;
            $('#shop_twelve0' + ic + '0').html(da.machine[i]);
            $('#shop_twelve1' + ic + '1').html(da.Job_name[i]);
            $('#shop_twelve2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_twelve3' + ic + '3').html("Arc On");
                  $('#shop_twelve3' + ic + '3').css({
                     'background-color': '#41e541',
                     'color': 'black'
                  });

               } else {
                  $('#shop_twelve3' + ic + '3').html("Arc Off");
                  $('#shop_twelve3' + ic + '3').css({
            ...
            $('#shop_twelve10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_twelve11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_twelve12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_twelve13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_twelve14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_twelve15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_twelve17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shoptwelve_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shoptwelve_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shoptwelve_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shoptwelve_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shoptwelve_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shoptwelve_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shoptwelve_wait_entry" + ic).css({
                  ...
                  $('#shoptwelve_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shoptwelve_wait_machine' + ic).html(da.machine[i]);
               $('#shoptwelve_wait_machine' + ic).show();
               $('#shoptwelve_wait_entry' + ic).show();
            } else {

               $('#shoptwelve_wait_machine' + ic).hide();
               $('#shoptwelve_wait_entry' + ic).hide();
            }
            ...
            $('#shop_twelve4' + ic + '4').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_twelve4' + ic + '4').css({
            ...
            $('#shop_twelve6' + ic + '6').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_twelve6' + ic + '6').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_twelve8' + ic + '8').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_twelve8' + ic + '8').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_twelve5' + ic + '5').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_twelve5' + ic + '5').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_twelve7' + ic + '7').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_twelve7' + ic + '7').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_twelve9' + ic + '9').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_twelve9' + ic + '9').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            if ($('#shop_twelve010').text() == '') {

            $('#shoptwelve_row1').hide();

         }

                  if ($('#shop_twelve020').text() == '') {
                              
            $('#shoptwelve_row2').hide();

         }

                  if ($('#shop_twelve030').text() == '') {
                              
            $('#shoptwelve_row3').hide();

         }

                  if ($('#shop_twelve040').text() == '') {
                              
            $('#shoptwelve_row4').hide();

         }


            if ($('#shop_twelve050').text() == '') {

            $('#shoptwelve_row5').hide();

         }


            if ($('#shop_twelve060').text() == '') {

            $('#shoptwelve_row6').hide();
...
function shoptwelve_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwelve&type=csv&file=live.csv&ip=" + shoptwelve_ip,
      async: true,
      success: function (data1) {
         ...
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
              ...
              $('#shop_twelve16' + ic + '16').html(ac.toFixed(2));
...
function shoptwelve_amp_graph(x) {
   ...
   shoptwelve_speed_amp_graph(x);


}

function shoptwelve_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shoptwelve_speed_volt_graph(x);


}

function shoptwelve_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shoptwelve_speed_gas_graph(x);


}

function shoptwelve_speed_amp_graph(x) {

   var machine = $("shop_twelve0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shoptwelve&type=csv&file=machine" + x + ".csv&ip=" + shoptwelve_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shoptwelve_poll1 == 0) {
            //////console.log("load");
            var options = {
               tooltip: {
                  shared: true,
...
function shoptwelve_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shoptwelve_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shoptwelve_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwelve&type=csv&file=down_reason.csv&ip=" + shoptwelve_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shoptwelver1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shoptwelver1_machine = [];
         var shoptwelver1_reason = [];
         var shoptwelver1_machine_unique = [];
         var shoptwelver1_reason_unique = [];
         var shoptwelver1_reason_data = [];
         var count_shoptwelver1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shoptwelver1_machine[count_shoptwelver1] = down_line[0];
               shoptwelver1_reason[count_shoptwelver1] = down_line[1];
               shoptwelver1_reason_data[count_shoptwelver1] = down_line[2];
               count_shoptwelver1++;
               data_a[count_shoptwelver1] = down_line;
            }
         }
         const s1 = new Set(shoptwelver1_machine);
         const s2 = new Set(shoptwelver1_reason);
         shoptwelver1_machine_unique = [...s1];
         shoptwelver1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shoptwelver1_machine_unique = shoptwelver1_machine_unique.sort(collator.compare);
         shoptwelver1_reason_unique = shoptwelver1_reason_unique.sort(collator.compare);
         var arshoptwelver11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shoptwelver1_machine_unique.length; i++) {
            for (var j = 0; j < shoptwelver1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shoptwelver1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shoptwelver1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shoptwelvedown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shoptwelvedown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shoptwelve_downtime");
         for (var i = -1; i < shoptwelver1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shoptwelver1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shoptwelver1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shoptwelver1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shoptwelver1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shoptwelver1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shoptwelver1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shoptwelve_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shoptwelver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwelver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwelve_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shoptwelver1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shoptwelver1_machine_unique.length; j++) {
               var aa = document.getElementById("shoptwelve_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shoptwelve_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shoptwelve_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwelve&type=per_hour&date="+shoptwelve_hr_date+"&shop="+shoptwelve_hr_shop+"&shift="+shoptwelve_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shoptwelvehr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shoptwelvehr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shoptwelve_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
