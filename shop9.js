...
var shopnine_poll1=0;
   if ($("#shopninecontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shopnine_poll1 = 0;
      //////console.log(shopnine_poll1);
   } else {
      shopnine_poll1 = 1;
      //////console.log(shopnine_poll1);
   }
...
var shopnine_machine_array = [];
var shopnine_hr_shop = "";
var shopnine_hr_shift = "";
var shopnine_hr_date = "";



   $(document).on('click', '.shop9_machine_name', function () {

      var shop=shopnine_name[0];
      var machine=$(this).text();
      var date=shopnine_hr_date;
      var shift_name=shopnine_hr_shift[0];
     
      var split_date=shopnine_prev_date.split("-");

       $("html").toggleClass("sidebar-left-opened");
      window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shopnine_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shopnine_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shopnine_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop9_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop9_machine_name font_16');
               td.setAttribute('id', 'shop_nine' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopnine_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopnine_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_nine' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopnine_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shopnine_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shopnine_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shopnine_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shopnine_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shopnine_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shopnine_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopnine&type=json&ip=" + shopnine_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shopnine_name = da.shop;
            shopnine_machine_array = da.machine;
            shopnine_prev_shift=da.prev_shift[0];
            shopnine_prev_date=da.prev_date[0];
            //console.log(shopnine_prev_date);
            $("#shopnine_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[9].text = da.shop;
            $('#shop_nine0' + ic + '0').html(da.machine[i]);
            ...
            shopnine_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shopnine_hr_shop = da.shop;
            shopnine_hr_shift = da.shift;
            $('#shop_nine0' + ic + '0').html(da.machine[i]);
            $('#shop_nine1' + ic + '1').html(da.Job_name[i]);
            $('#shop_nine2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_nine3' + ic + '3').html("Arc On");
                  $('#shop_nine3' + ic + '3').css({
            ...
            $('#shop_nine10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_nine11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_nine12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_nine13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_nine14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_nine15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_nine17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shopnine_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shopnine_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shopnine_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shopnine_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shopnine_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shopnine_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shopnine_wait_entry" + ic).css({
                  ...
                  $('#shopnine_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shopnine_wait_machine' + ic).html(da.machine[i]);
               $('#shopnine_wait_machine' + ic).show();
               $('#shopnine_wait_entry' + ic).show();
            } else {

               $('#shopnine_wait_machine' + ic).hide();
               $('#shopnine_wait_entry' + ic).hide();
            }
            ...
            if ($('#shop_nine010').text() == '') {

            $('#shopnine_row1').hide();

         }

                  if ($('#shop_nine020').text() == '') {
                              
            $('#shopnine_row2').hide();

         }

                  if ($('#shop_nine030').text() == '') {
                              
            $('#shopnine_row3').hide();

         }

                  if ($('#shop_nine040').text() == '') {
...
function shopnine_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopnine&type=csv&file=live.csv&ip=" + shopnine_ip,
      async: true,
      success: function (data1) {
         ...
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
               ...
               $('#shop_nine16' + ic + '16').html(ac.toFixed(2));
...
function shopnine_amp_graph(x) {
   ...
   shopnine_speed_amp_graph(x);


}

function shopnine_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopnine_speed_volt_graph(x);


}

function shopnine_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopnine_speed_gas_graph(x);


}

function shopnine_speed_amp_graph(x) {

   var machine = $("shop_nine0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shopnine&type=csv&file=machine" + x + ".csv&ip=" + shopnine_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shopnine_poll1 == 0) {
...
function shopnine_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopnine_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopnine_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopnine&type=csv&file=down_reason.csv&ip=" + shopnine_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shopniner1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shopniner1_machine = [];
         var shopniner1_reason = [];
         var shopniner1_machine_unique = [];
         var shopniner1_reason_unique = [];
         var shopniner1_reason_data = [];
         var count_shopniner1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shopniner1_machine[count_shopniner1] = down_line[0];
               shopniner1_reason[count_shopniner1] = down_line[1];
               shopniner1_reason_data[count_shopniner1] = down_line[2];
               count_shopniner1++;
               data_a[count_shopniner1] = down_line;
            }
         }
         const s1 = new Set(shopniner1_machine);
         const s2 = new Set(shopniner1_reason);
         shopniner1_machine_unique = [...s1];
         shopniner1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shopniner1_machine_unique = shopniner1_machine_unique.sort(collator.compare);
         shopniner1_reason_unique = shopniner1_reason_unique.sort(collator.compare);
         var arshopniner11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shopniner1_machine_unique.length; i++) {
            for (var j = 0; j < shopniner1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shopniner1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shopniner1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shopninedown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopninedown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shopnine_downtime");
         for (var i = -1; i < shopniner1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shopniner1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shopniner1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shopniner1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shopniner1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shopniner1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shopniner1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shopnine_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shopniner1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopniner1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopnine_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shopniner1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shopniner1_machine_unique.length; j++) {
               var aa = document.getElementById("shopnine_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shopnine_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shopnine_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopnine&type=per_hour&date="+shopnine_hr_date+"&shop="+shopnine_hr_shop+"&shift="+shopnine_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shopninehr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopninehr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shopnine_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
