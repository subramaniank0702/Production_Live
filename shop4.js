...
var shopfour_poll1=0;
   if ($("#shopfourcontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shopfour_poll1 = 0;
      //////console.log(shopfour_poll1);
   } else {
      shopfour_poll1 = 1;
      //////console.log(shopfour_poll1);
   }
...
var shopfour_machine_array = [];
var shopfour_hr_shop = "";
var shopfour_hr_shift = "";
var shopfour_hr_date = "";



   $(document).on('click', '.shop4_machine_name', function () {

      var shop=shopfour_name[0];
      var machine=$(this).text();
      var date=shopfour_hr_date;
      var shift_name=shopfour_hr_shift[0];
     
      var split_date=shopfour_prev_date.split("-");

       $("html").toggleClass("sidebar-left-opened");
        window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shopfour_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shopfour_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shopfour_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop4_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop4_machine_name font_16');
               td.setAttribute('id', 'shop_four' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopfour_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopfour_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_four' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopfour_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shopfour_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shopfour_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shopfour_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shopfour_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shopfour_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shopfour_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfour&type=json&ip=" + shopfour_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shopfour_name = da.shop;
            shopfour_machine_array = da.machine;
            shopfour_prev_shift=da.prev_shift[0];
            shopfour_prev_date=da.prev_date[0];
            //console.log(shopfour_prev_date);
            $("#shopfour_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[4].text = da.shop;
            $('#shop_four0' + ic + '0').html(da.machine[i]);
            ...
            shopfour_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shopfour_hr_shop = da.shop;
            shopfour_hr_shift = da.shift;
            $('#shop_four0' + ic + '0').html(da.machine[i]);
            $('#shop_four1' + ic + '1').html(da.Job_name[i]);
            $('#shop_four2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_four3' + ic + '3').html("Arc On");
                  $('#shop_four3' + ic + '3').css({
            ...
            $('#shop_four10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_four11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_four12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_four13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_four14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_four15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_four17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shopfour_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shopfour_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shopfour_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shopfour_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shopfour_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shopfour_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shopfour_wait_entry" + ic).css({
                  ...
                  $('#shopfour_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shopfour_wait_machine' + ic).html(da.machine[i]);
               $('#shopfour_wait_machine' + ic).show();
               $('#shopfour_wait_entry' + ic).show();
            } else {

               $('#shopfour_wait_machine' + ic).hide();
               $('#shopfour_wait_entry' + ic).hide();
            }
            ...
            if ($('#shop_four010').text() == '') {

            $('#shopfour_row1').hide();

         }

                  if ($('#shop_four020').text() == '') {
                              
            $('#shopfour_row2').hide();

         }

                  if ($('#shop_four030').text() == '') {
                              
            $('#shopfour_row3').hide();

         }

                  if ($('#shop_four040').text() == '') {
...
function shopfour_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfour&type=csv&file=live.csv&ip=" + shopfour_ip,
      async: true,
      success: function (data1) {
         ...
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
               ...
               $('#shop_four16' + ic + '16').html(ac.toFixed(2));
...
function shopfour_amp_graph(x) {
   ...
   shopfour_speed_amp_graph(x);


}

function shopfour_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopfour_speed_volt_graph(x);


}

function shopfour_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopfour_speed_gas_graph(x);


}

function shopfour_speed_amp_graph(x) {

   var machine = $("shop_four0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shopfour&type=csv&file=machine" + x + ".csv&ip=" + shopfour_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shopfour_poll1 == 0) {
...
function shopfour_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopfour_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopfour_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfour&type=csv&file=down_reason.csv&ip=" + shopfour_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shopfourr1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shopfourr1_machine = [];
         var shopfourr1_reason = [];
         var shopfourr1_machine_unique = [];
         var shopfourr1_reason_unique = [];
         var shopfourr1_reason_data = [];
         var count_shopfourr1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shopfourr1_machine[count_shopfourr1] = down_line[0];
               shopfourr1_reason[count_shopfourr1] = down_line[1];
               shopfourr1_reason_data[count_shopfourr1] = down_line[2];
               count_shopfourr1++;
               data_a[count_shopfourr1] = down_line;
            }
         }
         const s1 = new Set(shopfourr1_machine);
         const s2 = new Set(shopfourr1_reason);
         shopfourr1_machine_unique = [...s1];
         shopfourr1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shopfourr1_machine_unique = shopfourr1_machine_unique.sort(collator.compare);
         shopfourr1_reason_unique = shopfourr1_reason_unique.sort(collator.compare);
         var arshopfourr11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shopfourr1_machine_unique.length; i++) {
            for (var j = 0; j < shopfourr1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shopfourr1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shopfourr1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shopfourdown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopfourdown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shopfour_downtime");
         for (var i = -1; i < shopfourr1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shopfourr1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shopfourr1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shopfourr1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shopfourr1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shopfourr1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shopfourr1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shopfour_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shopfourr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfourr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfour_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shopfourr1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shopfourr1_machine_unique.length; j++) {
               var aa = document.getElementById("shopfour_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shopfour_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shopfour_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfour&type=per_hour&date="+shopfour_hr_date+"&shop="+shopfour_hr_shop+"&shift="+shopfour_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shopfourhr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopfourhr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shopfour_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
