...
var shopeighteen_poll1=0;
   if ($("#shopeighteencontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shopeighteen_poll1 = 0;
      //////console.log(shopeighteen_poll1);
   } else {
      shopeighteen_poll1 = 1;
      //////console.log(shopeighteen_poll1);
   }
...
var shopeighteen_machine_array = [];
var shopeighteen_hr_shop = "";
var shopeighteen_hr_shift = "";
var shopeighteen_hr_date = "";



   $(document).on('click', '.shop18_machine_name', function () {

      var shop=shopeighteen_name[0];
      var machine=$(this).text();
      var date=shopeighteen_hr_date;
      var shift_name=shopeighteen_hr_shift[0];
     
      var split_date=shop18_prev_date.split("-");

       $("html").toggleClass("sidebar-left-opened");
      window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shop18_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shopeighteen_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shopeighteen_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop18_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop18_machine_name font_16');
               td.setAttribute('id', 'shop_eighteen' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopeighteen_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopeighteen_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_eighteen' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopeighteen_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shopeighteen_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shopeighteen_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shopeighteen_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shopeighteen_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shopeighteen_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shopeighteen_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopeighteen&type=json&ip=" + shopeighteen_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shopeighteen_name = da.shop;
            shopeighteen_machine_array = da.machine;
            shop18_prev_shift=da.prev_shift[0];
            shop18_prev_date=da.prev_date[0];
            //console.log(shop18_prev_date);
            $("#shopeighteen_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[18].text = da.shop;
            $('#shop_eighteen0' + ic + '0').html(da.machine[i]);
            ...
            shopeighteen_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shopeighteen_hr_shop = da.shop;
            shopeighteen_hr_shift = da.shift;
            $('#shop_eighteen0' + ic + '0').html(da.machine[i]);
            $('#shop_eighteen1' + ic + '1').html(da.Job_name[i]);
            $('#shop_eighteen2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_eighteen3' + ic + '3').html("Arc On");
                  $('#shop_eighteen3' + ic + '3').css({
                     'background-color': '#41e541',
                     'color': 'black'
                  });

               } else {
                  $('#shop_eighteen3' + ic + '3').html("Arc Off");
                  $('#shop_eighteen3' + ic + '3').css({
            ...
            $('#shop_eighteen10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_eighteen11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_eighteen12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_eighteen13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_eighteen14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_eighteen15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_eighteen17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shopeighteen_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shopeighteen_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shopeighteen_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shopeighteen_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shopeighteen_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shopeighteen_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shopeighteen_wait_entry" + ic).css({
                  ...
                  $('#shopeighteen_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shopeighteen_wait_machine' + ic).html(da.machine[i]);
               $('#shopeighteen_wait_machine' + ic).show();
               $('#shopeighteen_wait_entry' + ic).show();
            } else {

               $('#shopeighteen_wait_machine' + ic).hide();
               $('#shopeighteen_wait_entry' + ic).hide();
            }
            ...
            $('#shop_eighteen4' + ic + '4').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_eighteen4' + ic + '4').css({
            ...
            $('#shop_eighteen6' + ic + '6').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_eighteen6' + ic + '6').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_eighteen8' + ic + '8').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_eighteen8' + ic + '8').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_eighteen5' + ic + '5').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_eighteen5' + ic + '5').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_eighteen7' + ic + '7').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_eighteen7' + ic + '7').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_eighteen9' + ic + '9').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_eighteen9' + ic + '9').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            if ($('#shop_eighteen010').text() == '') {

            $('#shopeighteen_row1').hide();

         }

                  if ($('#shop_eighteen020').text() == '') {
                              
            $('#shopeighteen_row2').hide();

         }

                  if ($('#shop_eighteen030').text() == '') {
                              
            $('#shopeighteen_row3').hide();

         }

                  if ($('#shop_eighteen040').text() == '') {
                              
            $('#shopeighteen_row4').hide();

         }


            if ($('#shop_eighteen050').text() == '') {

            $('#shopeighteen_row5').hide();

         }


            if ($('#shop_eighteen060').text() == '') {

            $('#shopeighteen_row6').hide();
...
function shopeighteen_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopeighteen&type=csv&file=live.csv&ip=" + shopeighteen_ip,
      async: true,
      success: function (data1) {
         ...
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
             ...
             $('#shop_eighteen16' + ic + '16').html(ac.toFixed(2));
...
function shopeighteen_amp_graph(x) {
   ...
   shopeighteen_speed_amp_graph(x);


}

function shopeighteen_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopeighteen_speed_volt_graph(x);


}

function shopeighteen_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopeighteen_speed_gas_graph(x);


}

function shopeighteen_speed_amp_graph(x) {

   var machine = $("shop_eighteen0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shopeighteen&type=csv&file=machine" + x + ".csv&ip=" + shopeighteen_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shopeighteen_poll1 == 0) {
            //////console.log("load");
            var options = {
               tooltip: {
                  shared: true,
...
function shopeighteen_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopeighteen_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopeighteen_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopeighteen&type=csv&file=down_reason.csv&ip=" + shopeighteen_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shopeighteenr1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shopeighteenr1_machine = [];
         var shopeighteenr1_reason = [];
         var shopeighteenr1_machine_unique = [];
         var shopeighteenr1_reason_unique = [];
         var shopeighteenr1_reason_data = [];
         var count_shopeighteenr1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shopeighteenr1_machine[count_shopeighteenr1] = down_line[0];
               shopeighteenr1_reason[count_shopeighteenr1] = down_line[1];
               shopeighteenr1_reason_data[count_shopeighteenr1] = down_line[2];
               count_shopeighteenr1++;
               data_a[count_shopeighteenr1] = down_line;
            }
         }
         const s1 = new Set(shopeighteenr1_machine);
         const s2 = new Set(shopeighteenr1_reason);
         shopeighteenr1_machine_unique = [...s1];
         shopeighteenr1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shopeighteenr1_machine_unique = shopeighteenr1_machine_unique.sort(collator.compare);
         shopeighteenr1_reason_unique = shopeighteenr1_reason_unique.sort(collator.compare);
         var arshopeighteenr11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shopeighteenr1_machine_unique.length; i++) {
            for (var j = 0; j < shopeighteenr1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shopeighteenr1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shopeighteenr1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shopeighteendown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopeighteendown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shopeighteen_downtime");
         for (var i = -1; i < shopeighteenr1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shopeighteenr1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shopeighteenr1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shopeighteenr1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shopeighteenr1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shopeighteenr1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shopeighteenr1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shopeighteen_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shopeighteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopeighteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopeighteen_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shopeighteenr1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shopeighteenr1_machine_unique.length; j++) {
               var aa = document.getElementById("shopeighteen_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shopeighteen_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shopeighteen_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopeighteen&type=per_hour&date="+shopeighteen_hr_date+"&shop="+shopeighteen_hr_shop+"&shift="+shopeighteen_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shopeighteenhr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopeighteenhr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shopeighteen_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
