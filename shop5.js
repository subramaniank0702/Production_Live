...
var shopfive_poll1=0;
   if ($("#shopfivecontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shopfive_poll1 = 0;
      //////console.log(shopfive_poll1);
   } else {
      shopfive_poll1 = 1;
      //////console.log(shopfive_poll1);
   }
...
var shopfive_machine_array = [];
var shopfive_hr_shop = "";
var shopfive_hr_shift = "";
var shopfive_hr_date = "";



   $(document).on('click', '.shop5_machine_name', function () {

      var shop=shopfive_name[0];
      var machine=$(this).text();
      var date=shopfive_hr_date;
      var shift_name=shopfive_hr_shift[0];
     
      var split_date=shopfive_prev_date.split("-");

       $("html").toggleClass("sidebar-left-opened");
      window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shopfive_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shopfive_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shopfive_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop5_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop5_machine_name font_16');
               td.setAttribute('id', 'shop_five' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopfive_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopfive_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_five' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopfive_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shopfive_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shopfive_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shopfive_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shopfive_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shopfive_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shopfive_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfive&type=json&ip=" + shopfive_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shopfive_name = da.shop;
            shopfive_machine_array = da.machine;
            shopfive_prev_shift=da.prev_shift[0];
            shopfive_prev_date=da.prev_date[0];
            //console.log(shopfive_prev_date);
            $("#shopfive_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[5].text = da.shop;
            $('#shop_five0' + ic + '0').html(da.machine[i]);
            ...
            shopfive_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shopfive_hr_shop = da.shop;
            shopfive_hr_shift = da.shift;
            $('#shop_five0' + ic + '0').html(da.machine[i]);
            $('#shop_five1' + ic + '1').html(da.Job_name[i]);
            $('#shop_five2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_five3' + ic + '3').html("Arc On");
                  $('#shop_five3' + ic + '3').css({
            ...
            $('#shop_five10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_five11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_five12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_five13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_five14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_five15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_five17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shopfive_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shopfive_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shopfive_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shopfive_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shopfive_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shopfive_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shopfive_wait_entry" + ic).css({
                  ...
                  $('#shopfive_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shopfive_wait_machine' + ic).html(da.machine[i]);
               $('#shopfive_wait_machine' + ic).show();
               $('#shopfive_wait_entry' + ic).show();
            } else {

               $('#shopfive_wait_machine' + ic).hide();
               $('#shopfive_wait_entry' + ic).hide();
            }
            ...
            if ($('#shop_five010').text() == '') {

            $('#shopfive_row1').hide();

         }

                  if ($('#shop_five020').text() == '') {
                              
            $('#shopfive_row2').hide();

         }

                  if ($('#shop_five030').text() == '') {
                              
            $('#shopfive_row3').hide();

         }

                  if ($('#shop_five040').text() == '') {
...
function shopfive_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfive&type=csv&file=live.csv&ip=" + shopfive_ip,
      async: true,
      success: function (data1) {
         var live_array = data1.split(/\r?\n|\r/);
         console.log(shopfive_ip);
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
               ...
               var ac=parseFloat(last_array[ic])-parseFloat(first_array[ic]);
                ...
                $('#shop_five16' + ic + '16').html(ac.toFixed(2));
...
function shopfive_amp_graph(x) {
   ...
   shopfive_speed_amp_graph(x);


}

function shopfive_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopfive_speed_volt_graph(x);


}

function shopfive_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopfive_speed_gas_graph(x);


}

function shopfive_speed_amp_graph(x) {

   var machine = $("shop_five0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shopfive&type=csv&file=machine" + x + ".csv&ip=" + shopfive_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shopfive_poll1 == 0) {
...
function shopfive_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopfive_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopfive_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfive&type=csv&file=down_reason.csv&ip=" + shopfive_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shopfiver1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shopfiver1_machine = [];
         var shopfiver1_reason = [];
         var shopfiver1_machine_unique = [];
         var shopfiver1_reason_unique = [];
         var shopfiver1_reason_data = [];
         var count_shopfiver1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shopfiver1_machine[count_shopfiver1] = down_line[0];
               shopfiver1_reason[count_shopfiver1] = down_line[1];
               shopfiver1_reason_data[count_shopfiver1] = down_line[2];
               count_shopfiver1++;
               data_a[count_shopfiver1] = down_line;
            }
         }
         const s1 = new Set(shopfiver1_machine);
         const s2 = new Set(shopfiver1_reason);
         shopfiver1_machine_unique = [...s1];
         shopfiver1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shopfiver1_machine_unique = shopfiver1_machine_unique.sort(collator.compare);
         shopfiver1_reason_unique = shopfiver1_reason_unique.sort(collator.compare);
         var arshopfiver11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shopfiver1_machine_unique.length; i++) {
            for (var j = 0; j < shopfiver1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shopfiver1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shopfiver1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shopfivedown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopfivedown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shopfive_downtime");
         for (var i = -1; i < shopfiver1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shopfiver1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shopfiver1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shopfiver1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shopfiver1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shopfiver1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shopfiver1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shopfive_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shopfiver1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopfiver1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopfive_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shopfiver1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shopfiver1_machine_unique.length; j++) {
               var aa = document.getElementById("shopfive_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shopfive_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shopfive_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopfive&type=per_hour&date="+shopfive_hr_date+"&shop="+shopfive_hr_shop+"&shift="+shopfive_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shopfivehr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopfivehr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shopfive_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
