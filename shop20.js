...
var shoptwenty_poll1=0;
   if ($("#shoptwentycontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shoptwenty_poll1 = 0;
      //////console.log(shoptwenty_poll1);
   } else {
      shoptwenty_poll1 = 1;
      //////console.log(shoptwenty_poll1);
   }
...
var shoptwenty_machine_array = [];
var shoptwenty_hr_shop = "";
var shoptwenty_hr_shift = "";
var shoptwenty_hr_date = "";



   $(document).on('click', '.shop20_machine_name', function () {

      var shop=shoptwenty_name[0];
      var machine=$(this).text();
      var date=shoptwenty_hr_date;
      var shift_name=shoptwenty_hr_shift[0];
     
      var split_date=shop20_prev_date.split("-");

       $("html").toggleClass("sidebar-left-opened");
      window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shop20_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shoptwenty_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shoptwenty_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop20_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop20_machine_name font_16');
               td.setAttribute('id', 'shop_twenty' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shoptwenty_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shoptwenty_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_twenty' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shoptwenty_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shoptwenty_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shoptwenty_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shoptwenty_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shoptwenty_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shoptwenty_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shoptwenty_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwenty&type=json&ip=" + shoptwenty_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shoptwenty_name = da.shop;
            shoptwenty_machine_array = da.machine;
            shop20_prev_shift=da.prev_shift[0];
            shop20_prev_date=da.prev_date[0];
            //console.log(shop20_prev_date);
            $("#shoptwenty_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[20].text = da.shop;
            $('#shop_twenty0' + ic + '0').html(da.machine[i]);
            ...
            shoptwenty_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shoptwenty_hr_shop = da.shop;
            shoptwenty_hr_shift = da.shift;
            $('#shop_twenty0' + ic + '0').html(da.machine[i]);
            $('#shop_twenty1' + ic + '1').html(da.Job_name[i]);
            $('#shop_twenty2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_twenty3' + ic + '3').html("Arc On");
                  $('#shop_twenty3' + ic + '3').css({
                     'background-color': '#41e541',
                     'color': 'black'
                  });

               } else {
                  $('#shop_twenty3' + ic + '3').html("Arc Off");
                  $('#shop_twenty3' + ic + '3').css({
            ...
            $('#shop_twenty10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_twenty11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_twenty12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_twenty13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_twenty14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_twenty15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_twenty17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shoptwenty_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shoptwenty_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shoptwenty_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shoptwenty_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shoptwenty_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shoptwenty_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shoptwenty_wait_entry" + ic).css({
                  ...
                  $('#shoptwenty_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shoptwenty_wait_machine' + ic).html(da.machine[i]);
               $('#shoptwenty_wait_machine' + ic).show();
               $('#shoptwenty_wait_entry' + ic).show();
            } else {

               $('#shoptwenty_wait_machine' + ic).hide();
               $('#shoptwenty_wait_entry' + ic).hide();
            }
            ...
            $('#shop_twenty4' + ic + '4').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_twenty4' + ic + '4').css({
            ...
            $('#shop_twenty6' + ic + '6').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_twenty6' + ic + '6').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_twenty8' + ic + '8').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_twenty8' + ic + '8').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_twenty5' + ic + '5').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_twenty5' + ic + '5').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_twenty7' + ic + '7').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_twenty7' + ic + '7').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_twenty9' + ic + '9').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_twenty9' + ic + '9').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            if ($('#shop_twenty010').text() == '') {

            $('#shoptwenty_row1').hide();

         }

                  if ($('#shop_twenty020').text() == '') {
                              
            $('#shoptwenty_row2').hide();

         }

                  if ($('#shop_twenty030').text() == '') {
                              
            $('#shoptwenty_row3').hide();

         }

                  if ($('#shop_twenty040').text() == '') {
                              
            $('#shoptwenty_row4').hide();

         }


            if ($('#shop_twenty050').text() == '') {

            $('#shoptwenty_row5').hide();

         }


            if ($('#shop_twenty060').text() == '') {

            $('#shoptwenty_row6').hide();
...
function shoptwenty_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwenty&type=csv&file=live.csv&ip=" + shoptwenty_ip,
      async: true,
      success: function (data1) {
         ...
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
               ...
               $('#shop_twenty16' + ic + '16').html(ac.toFixed(2));
...
function shoptwenty_amp_graph(x) {
   ...
   shoptwenty_speed_amp_graph(x);


}

function shoptwenty_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shoptwenty_speed_volt_graph(x);


}

function shoptwenty_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shoptwenty_speed_gas_graph(x);


}

function shoptwenty_speed_amp_graph(x) {

   var machine = $("shop_twenty0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shoptwenty&type=csv&file=machine" + x + ".csv&ip=" + shoptwenty_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shoptwenty_poll1 == 0) {
            //////console.log("load");
            var options = {
               tooltip: {
                  shared: true,
...
function shoptwenty_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shoptwenty_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shoptwenty_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwenty&type=csv&file=down_reason.csv&ip=" + shoptwenty_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shoptwentyr1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shoptwentyr1_machine = [];
         var shoptwentyr1_reason = [];
         var shoptwentyr1_machine_unique = [];
         var shoptwentyr1_reason_unique = [];
         var shoptwentyr1_reason_data = [];
         var count_shoptwentyr1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shoptwentyr1_machine[count_shoptwentyr1] = down_line[0];
               shoptwentyr1_reason[count_shoptwentyr1] = down_line[1];
               shoptwentyr1_reason_data[count_shoptwentyr1] = down_line[2];
               count_shoptwentyr1++;
               data_a[count_shoptwentyr1] = down_line;
            }
         }
         const s1 = new Set(shoptwentyr1_machine);
         const s2 = new Set(shoptwentyr1_reason);
         shoptwentyr1_machine_unique = [...s1];
         shoptwentyr1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shoptwentyr1_machine_unique = shoptwentyr1_machine_unique.sort(collator.compare);
         shoptwentyr1_reason_unique = shoptwentyr1_reason_unique.sort(collator.compare);
         var arshoptwentyr11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shoptwentyr1_machine_unique.length; i++) {
            for (var j = 0; j < shoptwentyr1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shoptwentyr1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shoptwentyr1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shoptwentydown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shoptwentydown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shoptwenty_downtime");
         for (var i = -1; i < shoptwentyr1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shoptwentyr1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shoptwentyr1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shoptwentyr1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shoptwentyr1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shoptwentyr1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shoptwentyr1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shoptwenty_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shoptwentyr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shoptwentyr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shoptwenty_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shoptwentyr1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shoptwentyr1_machine_unique.length; j++) {
               var aa = document.getElementById("shoptwenty_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shoptwenty_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shoptwenty_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shoptwenty&type=per_hour&date="+shoptwenty_hr_date+"&shop="+shoptwenty_hr_shop+"&shift="+shoptwenty_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shoptwentyhr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shoptwentyhr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shoptwenty_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
