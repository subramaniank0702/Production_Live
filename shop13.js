...
var shopthirteen_poll1=0;
   if ($("#shopthirteencontainer .apexcharts-legend-series ").attr('data:collapsed') === 'false') {
      shopthirteen_poll1 = 0;
      //////console.log(shopthirteen_poll1);
   } else {
      shopthirteen_poll1 = 1;
      //////console.log(shopthirteen_poll1);
   }
...
var shopthirteen_machine_array = [];
var shopthirteen_hr_shop = "";
var shopthirteen_hr_shift = "";
var shopthirteen_hr_date = "";



   $(document).on('click', '.shop13_machine_name', function () {

      var shop=shopthirteen_name[0];
      var machine=$(this).text();
      var date=shopthirteen_hr_date;
      var shift_name=shopthirteen_hr_shift[0];
     
      var split_date=shop13_prev_date.split("-");

     //  $("html").toggleClass("sidebar-left-opened");
      //window.open(basepath + '10.30.0.56/LWSoft/LWSoft_machine_overview/machine_live.php?'+ user_name +'#'+ shop +'#'+ machine +'#'+ split_date[2]+"-"+split_date[1]+"-"+split_date[0] +'#'+ shop13_prev_shift +'#','_self');
...
$(document).ready(function () {
   ...
   var body = document.getElementById("shopthirteen_realtime");
   for (var i = 0; i <= 6; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute('id', 'shopthirteen_row' + i);
      for (var j = 0; j <= 20; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               var cellText = document.createTextNode("Machine");
               th.setAttribute('class', 'shop13_machine_name');
            ...
            if (j == 0) {
               ...
               var cellText = document.createTextNode("Machine");
               td.setAttribute('class', 'shop13_machine_name font_16');
               td.setAttribute('id', 'shop_thirteen' + j + i + j);
            ...
            if (j == 3) {
              ...
              if (j == 4) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopthirteen_amp_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 5) {
               var td = document.createElement("td");
            ...
            if (j == 6) {
               ...
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopthirteen_volt_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
            if (j == 7) {
               var td = document.createElement("td");
            ...
            if (j == 8) {
               ...
               td.setAttribute('id', 'shop_thirteen' + j + i + j);
               var add_tab = i + 0;
               td.setAttribute('onclick', ' shopthirteen_gas_graph(' + add_tab + ');');
               tr.appendChild(td);
            }
   ...
   var table = document.createElement("table");
   var tblBody = document.createElement("tbody");
   table.id = "shopthirteen_wait";
   table.setAttribute("class", "inp");
   var body = document.getElementById("shopthirteen_wait_down");
   for (var i = 0; i < 2; i++) {
      var tr = document.createElement("tr");
      tr.setAttribute("id", 'shopthirteen_wait_row' + i)
      for (var j = 0; j < 7; j++) {
         if (i == 0) {
            if (j == 0) {
               var th = document.createElement("TH");
               ...
               var th = document.createElement("TH");
               th.setAttribute("id", 'shopthirteen_wait_machine' + j);
            ...
            if (j == 0) {
               ...
               var td = document.createElement("td");
               td.setAttribute('id', 'shopthirteen_wait_entry' + j);
               tr.appendChild(td);
            }
         }
      }
...
function shopthirteen_get_live_data() {
   ...
   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopthirteen&type=json&ip=" + shopthirteen_ip,
      dataType: "json",
      async: true,
      success: function (da) {
//console.log(da);``
         for (var i = 0; i < 6; i++) {
            var ic = i + 1;
            shopthirteen_name = da.shop;
            shopthirteen_machine_array = da.machine;
            shop13_prev_shift=da.prev_shift[0];
            shop13_prev_date=da.prev_date[0];
            //console.log(shop13_prev_date);
            $("#shopthirteen_lbl").html("Shop :- " + da.shop);
            document.getElementById("reason_sel_down").options[13].text = da.shop;
            $('#shop_thirteen0' + ic + '0').html(da.machine[i]);
            ...
            shopthirteen_hr_date = change_date[2] + "-" + change_date[1] + "-" + change_date[0];
            shopthirteen_hr_shop = da.shop;
            shopthirteen_hr_shift = da.shift;
            $('#shop_thirteen0' + ic + '0').html(da.machine[i]);
            $('#shop_thirteen1' + ic + '1').html(da.Job_name[i]);
            $('#shop_thirteen2' + ic + '2').html(da.Operator[i]);

            if (da.machine_status[i] == 1) {
               if (da.weld_status[i] == 1) {
                  $('#shop_thirteen3' + ic + '3').html("Arc On");
                  $('#shop_thirteen3' + ic + '3').css({
                     'background-color': '#41e541',
                     'color': 'black'
                  });

               } else {
                  $('#shop_thirteen3' + ic + '3').html("Arc Off");
                  $('#shop_thirteen3' + ic + '3').css({
            ...
            $('#shop_thirteen10' + ic + '10').html(parseFloat(da.tarctime[i]).toFixed(2));
            $('#shop_thirteen11' + ic + '11').html(parseFloat(da.perarcon[i]).toFixed(2));
            $('#shop_thirteen12' + ic + '12').html(parseFloat(downtime).toFixed(2));
            $('#shop_thirteen13' + ic + '13').html(parseFloat(da.mc_off[i]).toFixed(2));
            $('#shop_thirteen14' + ic + '14').html(parseFloat(da.job_count[i]).toFixed(0) + ' [ ' + parseFloat(da.weld_count[i]).toFixed(0) + ' ]');
            $('#shop_thirteen15' + ic + '15').html(parseFloat(da.weld_dep[i]).toFixed(2));
            $('#shop_thirteen17' + ic + '17').html(parseFloat(da.mc_cost_twc[i]).toFixed(2));

            //*********************waiting for downtime entry********************
           

            $('#shopthirteen_wait_entry1').html(parseFloat(da.wait_timer[0]).toFixed(2));
            $('#shopthirteen_wait_entry2').html(parseFloat(da.wait_timer[1]).toFixed(2));
            $('#shopthirteen_wait_entry3').html(parseFloat(da.wait_timer[2]).toFixed(2));
            $('#shopthirteen_wait_entry4').html(parseFloat(da.wait_timer[3]).toFixed(2));
            $('#shopthirteen_wait_entry5').html(parseFloat(da.wait_timer[4]).toFixed(2));
            $('#shopthirteen_wait_entry6').html(parseFloat(da.wait_timer[5]).toFixed(2));

            if ((da.machine[i] != 'N/A')) {
               if (da.wait_timer[i] != 0) {
                  $("#shopthirteen_wait_entry" + ic).css({
                  ...
                  $('#shopthirteen_wait_entry' + ic).stop(true, true).fadeTo(100, 1);
               }
            }

 if ((da.machine[i] != 'N/A')&&(da.machine[i] > '')) {
               $('#shopthirteen_wait_machine' + ic).html(da.machine[i]);
               $('#shopthirteen_wait_machine' + ic).show();
               $('#shopthirteen_wait_entry' + ic).show();
            } else {

               $('#shopthirteen_wait_machine' + ic).hide();
               $('#shopthirteen_wait_entry' + ic).hide();
            }
            ...
            $('#shop_thirteen4' + ic + '4').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_thirteen4' + ic + '4').css({
            ...
            $('#shop_thirteen6' + ic + '6').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_thirteen6' + ic + '6').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_thirteen8' + ic + '8').html(value);
            if ((value < upper) && (value > lower)) {
               $('#shop_thirteen8' + ic + '8').css({
                  'background-color': 'lime',
                  'color': 'black'
               });
            } else if ((value == 0) && (0 == value)) {
            ...
            $('#shop_thirteen5' + ic + '5').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_thirteen5' + ic + '5').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_thirteen7' + ic + '7').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_thirteen7' + ic + '7').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            $('#shop_thirteen9' + ic + '9').html(value);
            if ((value > 0) && (value < upper)) {
               $('#shop_thirteen9' + ic + '9').css({
                  'background-color': 'yellow',
                  'color': 'black'
               });
            ...
            if ($('#shop_thirteen010').text() == '') {

            $('#shopthirteen_row1').hide();

         }

                  if ($('#shop_thirteen020').text() == '') {
                              
            $('#shopthirteen_row2').hide();

         }

                  if ($('#shop_thirteen030').text() == '') {
                              
            $('#shopthirteen_row3').hide();

         }

                  if ($('#shop_thirteen040').text() == '') {
                              
            $('#shopthirteen_row4').hide();

         }


            if ($('#shop_thirteen050').text() == '') {

            $('#shopthirteen_row5').hide();

         }


            if ($('#shop_thirteen060').text() == '') {

            $('#shopthirteen_row6').hide();
...
function shopthirteen_hr_production() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopthirteen&type=csv&file=live.csv&ip=" + shopthirteen_ip,
      async: true,
         ...
         if (live_array.length - 1 < 60) {
            ...
            for (var i = 0; i < 6; i++) {
               ...
               var ac=parseFloat(last_array[ic])-parseFloat(first_array[ic]);
                ...
                $('#shop_thirteen16' + ic + '16').html(ac.toFixed(2));
...
function shopthirteen_amp_graph(x) {
   ...
   shopthirteen_speed_amp_graph(x);


}

function shopthirteen_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopthirteen_speed_volt_graph(x);


}

function shopthirteen_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
   ...
   shopthirteen_speed_gas_graph(x);


}

function shopthirteen_speed_amp_graph(x) {

   var machine = $("shop_thirteen0" + x + "0").html();
   //console.log("yes");
   $.ajax({
      type: "Get",
      url: window.location.protocol + "classes/productionlive.php?area=shopthirteen&type=csv&file=machine" + x + ".csv&ip=" + shopthirteen_ip,
      // url: "live_data.csv",
      async: true,
      success: function (data1) {
         if (shopthirteen_poll1 == 0) {
            //////console.log("load");
            var options = {
               tooltip: {
                  shared: true,
...
function shopthirteen_speed_volt_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopthirteen_speed_gas_graph(x) {
   var modal = document.getElementById("speed_modal");
   var btn = document.getElementById("clo2");
   modal.style.display = "block";
   time_elps = 1;
...
function shopthirteen_downtime() {

   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopthirteen&type=csv&file=down_reason.csv&ip=" + shopthirteen_ip,
      // url: "energy_record.csv",
      async: true,
      success: function (data1) {
         var count_shopthirteenr1 = 0.0;
         var machine1_arr = [];
         var machine2_arr = [];
         var machine3_arr = [];
         var machine4_arr = [];
         ...
         var machine14_cou = [];
         var machine15_cou = [];
         var shopthirteenr1_machine = [];
         var shopthirteenr1_reason = [];
         var shopthirteenr1_machine_unique = [];
         var shopthirteenr1_reason_unique = [];
         var shopthirteenr1_reason_data = [];
         var count_shopthirteenr1 = 0.0;
         var data_a = [];
         ...
         for (var i = 1; i < down_data.length - 1; i++) {
            ...
            if (down_line[0] !== "N/A") {
               shopthirteenr1_machine[count_shopthirteenr1] = down_line[0];
               shopthirteenr1_reason[count_shopthirteenr1] = down_line[1];
               shopthirteenr1_reason_data[count_shopthirteenr1] = down_line[2];
               count_shopthirteenr1++;
               data_a[count_shopthirteenr1] = down_line;
            }
         }
         const s1 = new Set(shopthirteenr1_machine);
         const s2 = new Set(shopthirteenr1_reason);
         shopthirteenr1_machine_unique = [...s1];
         shopthirteenr1_reason_unique = [...s2];
         var collator = new Intl.Collator(undefined, {
            numeric: true,
            sensitivity: 'base'
         });
         shopthirteenr1_machine_unique = shopthirteenr1_machine_unique.sort(collator.compare);
         shopthirteenr1_reason_unique = shopthirteenr1_reason_unique.sort(collator.compare);
         var arshopthirteenr11 = [];
         var add = 0.0;
         var count = 0;
         for (var i = 0; i < shopthirteenr1_machine_unique.length; i++) {
            for (var j = 0; j < shopthirteenr1_reason_unique.length; j++) {
               for (var k = 1; k < data_a.length; k++) {
                  if (data_a[k] != undefined) {
                     var chk1 = shopthirteenr1_machine_unique[i].localeCompare(data_a[k][0]);
                     var chk2 = shopthirteenr1_reason_unique[j].localeCompare(data_a[k][1]);
                     if ((chk1 == 0) && (chk2 == 0)) {
         ...
         $("#shopthirteendown2").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopthirteendown2";
         table.setAttribute("class", "tab2");
         var body = document.getElementById("shopthirteen_downtime");
         for (var i = -1; i < shopthirteenr1_reason_unique.length + 1; i++) {
            var tr = document.createElement("tr");
            if (i == -1) {
               tr.setAttribute("style", "position: sticky;top: 0;word-wrap: break-word;");
            }
            for (var j = 0; j <= shopthirteenr1_machine_unique.length + 1; j++) {
               if (i == -1) {
                  if (j == 0) {
                     var th = document.createElement("TH");
                     ...
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  } else if (shopthirteenr1_machine_unique.length >= j) {
                     var th = document.createElement("TH");
                     var cellText = document.createTextNode(shopthirteenr1_machine_unique[j - 1] + " " + ":" + "Downtime Min" + " " + "[" + "Count]");
                     th.appendChild(cellText);
                     th.setAttribute("style", "position: sticky;top: 0;");
                     tr.appendChild(th);
                  } else {
               ...
               } else if (shopthirteenr1_reason_unique.length > i) {
                  if (j == 1) {
                     var th = document.createElement("th");
                     var cellText = document.createTextNode(shopthirteenr1_reason_unique[i]);
                     th.appendChild(cellText);
                     tr.appendChild(th);
                  }
                  if (j == 2) {
                  ...
                  if (j >= shopthirteenr1_machine_unique.length + 1) {
                     var th = document.createElement("td");
                     var trd = j - 1;
                     th.id = "shopthirteen_tbw2" + i + trd;
                     var cellText = document.createTextNode("");
                  ...
                  if ((j == 1) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                        if (a > " ") {
                  ...
                  if ((j == 2) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 3) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 4) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 5) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 6) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 7) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 8) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 9) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 10) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 11) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
                  ...
                  if ((j == 12) && (j < shopthirteenr1_machine_unique.length + 1)) {
                     var add = 0;
                     for (var dd = 0; dd < shopthirteenr1_reason_unique.length; dd++) {
                        var cd = j - 1;
                        var a = document.getElementById('shopthirteen_tbw2' + dd + cd).innerHTML;
         ...
         for (var i = 0; i < shopthirteenr1_reason_unique.length + 1; i++) {
            var add = 0.0;
            for (var j = 0; j < shopthirteenr1_machine_unique.length; j++) {
               var aa = document.getElementById("shopthirteen_tbw2" + i + j).innerHTML;
               if (aa > " ") {
                  add = add + parseFloat(aa);
               }
            }
            document.getElementById("shopthirteen_tbw2" + i + j).innerHTML = add.toFixed(0);
         }
      }
   });
...
function shopthirteen_per_hour() {


   $.ajax({
      type: "Get",
      url: "classes/productionlive.php?area=shopthirteen&type=per_hour&date="+shopthirteen_hr_date+"&shop="+shopthirteen_hr_shop+"&shift="+shopthirteen_hr_shift+"&shop_no=1",
      async: true,
      dataType: "json",
      success: function (data1) {
         $("#shopthirteenhr_table").remove();
         var table = document.createElement("table");
         var tblBody = document.createElement("tbody");
         table.id = "shopthirteenhr_table";
         table.setAttribute("class", "inp");
         var body = document.getElementById("shopthirteen_hr");
         var tr = document.createElement("tr");
         var th = document.createElement("TH");
         var cellText = document.createTextNode("Machine");
         th.appendChild(cellText);
